...
runners.forEach(function (fna) {
  fullNames.push(`${fna.first_name} ${fna.last_name}`);
});
console.log(fullNames);
// ==== Challenge 2: Use .map() ====
...
firstNamesAllCaps=runners.map(function(fname){
  return(fname.first_name.toUpperCase());
});
console.log(firstNamesAllCaps);
...
runnersLargeSizeShirt=runners.filter(function(lshirt){
  if(lshirt.shirt_size==="L"){
    return true;
  } 
});
console.log(runnersLargeSizeShirt);
...
ticketPriceTotal=runners.reduce(function(total,donat){
  return total +  donat.donation;
},0);
